...
- **实时状态展示** — 公共状态页实时显示所有服务运行状况，含 90 天在线率矩阵与延迟分位数（P95 / P99）
- **故障事件管理** — 事件创建、进度更新、影响等级划分，自动关联服务状态
- **维护计划管理** — 计划内停机维护预告与展示，支持按天 / 周 / 月周期重复
- **月度 SLA 报告** — 按月汇总可用率、事件与响应时间，历史月份自动归档长期留存，可每月自动发邮件
- **国际化** — 公开页支持中文 / 英文切换（自动跟随浏览器，页脚可手动切换）；RSS 与邮件模板语言由 `LANG` 配置
