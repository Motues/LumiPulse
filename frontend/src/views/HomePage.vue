...
<template>
  <nav class="mt-4" style="background-color: var(--bg-color);">
    <div class="max-w-[1000px] mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <div class="flex items-center gap-3">
        <a v-if="emailEnabled" href="#" @click.prevent="openSubscribe" class="header-btn px-4 py-2 text-sm font-medium rounded-lg">订阅更新</a>
        <div class="relative">
          <button
  ...
  <main class="max-w-[1000px] mx-auto px-6 py-8">
    ...
    <template v-else-if="summary">
      ...
      <section v-if="incidents.length > 0" class="mb-8">
        ...
        <div v-for="inc in incidents" :key="inc.id" class="rounded-lg p-6 mb-4" style="border: 1px solid var(--button-border-color);">
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-2">
            ...
            <div class="text-sm font-medium mt-1 sm:mt-0" style="color: var(--text-color); opacity: 0.7;">{{ formatDate(inc.createdAt) }}</div>
          </div>
          <p v-if="inc.updates && inc.updates.length > 0" class="text-sm mb-6" style="color: var(--text-color); opacity: 0.5;">
            {{ inc.updates[inc.updates.length - 1].content }}
          </p>
          <div v-if="inc.updates && inc.updates.length > 0" class="relative">
            <ul class="space-y-4 relative z-10">
              <li v-for="upd in [...inc.updates].reverse()" :key="upd.id" class="relative pl-4">
  ...
  <Teleport to="body">
    <div v-if="showSubscribeModal" class="fixed inset-0 z-50 flex items-center justify-center" @click.self="closeSubscribeModal">
      ...
      <div class="relative rounded-xl shadow-xl p-6 w-full max-w-sm mx-4" style="background-color: var(--bg-color); border: 1px solid var(--button-border-color);">
        <h3 class="text-base font-bold mb-1" style="color: var(--text-color);">订阅状态通知</h3>
        <p class="text-sm mb-4" style="color: var(--text-color); opacity: 0.5;">获取服务状态变更和事件通知。</p>
        ...
        <button
          ...
          class="mt-4 w-full px-4 py-2 text-sm font-medium rounded-lg modal-close-btn"
        >
          关闭
        </button>
      </div>
  ...
  <footer style="background-color: var(--bg-color);" class="mt-4">
    <div class="max-w-[1000px] mx-auto px-6 py-8 flex flex-col md:flex-row justify-between items-center gap-4">
      ...
      <div class="flex items-center gap-6 text-sm">
        <a href="/dashboard" class="footer-link transition-opacity">管理后台</a>
      </div>
    </div>
  </footer>
</template>
